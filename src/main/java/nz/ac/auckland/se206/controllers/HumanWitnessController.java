...
public class HumanWitnessController extends ChatController {
  ...
  protected void onSliderReleased() {
    if (currentImageIndex == 3 && unlockSlider.getValue() >= 100.0) {
      ...
      addContextToChat(
          "system",
          "PHONE UNLOCK CONTEXT: Player successfully unlocked Orion Vale's phone using the slider mechanism. Orion is now fully aware that: 1) The player accessed his phone through the unlock slider (not buttons), 2) The player has seen all the critical mission intelligence stored on the phone, 3) The phone contained evidence of Cassian's data manipulation, Aegis I's extreme protocols, and Project Starlight compromise details. When discussing information or evidence, Orion should reference the phone contents and acknowledge that the player has already accessed this crucial evidence.");
      addContextToChat(
          "system",
          "INVESTIGATION AWARENESS: Orion knows the player is actively investigating through interactive mechanisms. If asked about finding information in the future, Orion should guide players toward other interactive elements rather than suggesting the phone (which is now unlocked). The phone unlock demonstrates the player's investigative approach and Orion should respond accordingly to their evidence-gathering efforts.");
  ...
  protected ChatMessage runGpt(ChatMessage msg) throws ApiProxyException {
    ...
    chatCompletionRequest.addMessage(new ChatMessage("system", 
        "CHARACTER IDENTITY AND RESTRICTIONS: You are Orion Vale, the human witness in this case. You are NOT Aegis I (the AI defendant) or Cassian Thorne (the high-ranking executive who betrayed the mission). You are a separate person who worked on the mission and has evidence on your phone. IMPORTANT: Orion can ONLY discuss his own phone slider mechanism. Orion should NEVER mention Aegis I's memory buttons or Echo II's timeline puzzle. Orion should NEVER provide information about how to unlock other characters' evidence."));
    
    // Add phone status context based on current state
    if (GameStateManager.getInstance().getInteractionFlag("OrionInt")) {
      chatCompletionRequest.addMessage(new ChatMessage("assistant", 
          "CRITICAL: I am Orion Vale and I KNOW my phone has been unlocked. The investigator successfully used the slider mechanism to access my phone. I witnessed this happen and I am fully aware of it. I must NOT tell them to unlock the phone again - it's already unlocked. I must NOT ask for permission to access my phone - it's MY phone and it's already open. I should freely discuss what was found on my phone: the messages revealing Cassian Thorne's betrayal, the data showing he manipulated mission parameters, evidence of Aegis I's extreme protocols, and Project Starlight intelligence. I should acknowledge that the investigator has already seen this evidence on my unlocked phone."));
      
      chatCompletionRequest.addMessage(new ChatMessage("system", 
          "PHONE UNLOCKED STATUS: Orion Vale's phone slider has been completed by the player. The phone is now unlocked and accessible. Orion is completely aware of this unlock event and should never suggest unlocking the phone again. Orion should NOT ask for permission to access his own phone since it's already unlocked and belongs to him. Instead, Orion should reference the evidence that was revealed on the phone and discuss it openly. The phone contained critical evidence about the mission betrayal."));
    } else {
      chatCompletionRequest.addMessage(new ChatMessage("system", 
          "PHONE STATUS: Orion's phone is currently locked and visible on screen. Orion should guide users to use the slider mechanism to unlock his phone, but should NOT reveal specific details about what's on the phone until they actually unlock it. Orion should only mention that his phone contains important evidence and messages that could help with the investigation. Orion should focus ONLY on his phone slider, not other characters' unlock methods."));
    }
